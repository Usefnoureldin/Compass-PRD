...
const InlineChecklistRow: React.FC<InlineChecklistRowProps> = ({ item, onToggle }) => {
  ...
  const indent = isHeading
    ? Math.max(0, (item.headingLevel ?? 1) - 2) * 8
    : 16 + (item.indent ?? 0) * 8;
    ...
    >
      ...
      <span
        className={cn(
          ...
          isHeading && (item.headingLevel ?? 9) <= 2 && "text-xs",
          isHeading && (item.headingLevel ?? 0) >= 4 && "text-[10px] text-muted-foreground uppercase tracking-wide",
